                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            <tr v-for="course in courses" :key="course.id">
                                ...
                                <td>
                                  ...
                                  <button v-on:click="destroy(course.id)" type="button" class="flex items-center px-5 py-2.5 font-medium tracking-wide text-black capitalize rounded-md  hover:bg-red-200 hover:fill-current hover:text-red-600  focus:outline-none  transition duration-300 transform active:scale-95 ease-in-out">
...
export default {
  ...
  methods: {
    ...
    destroy(id){
      this.errors = null;
      this.$http.delete('http://students.test/api/courses/' + id)
      .then(response => {
